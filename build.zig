...
fn addWriteFileCommand(b: *std.Build, destination: []const u8, contents: []const u8) *std.Build.Step.Run {
    // Keep both values out of the shell source. In particular, Git Bash
    // consumes backslashes from an interpolated native Windows path.
    const command = b.addSystemCommand(&.{ "sh", "-c", "printf '%s' \"$1\" > \"$2\"", "write-file" });
    command.addArg(contents);
    command.addArg(destination);
    return command;
}
...
fn bootstrapDepotTools(b: *std.Build, depot_tools_dir: []const u8) !*std.Build.Step {
    ...
    const write_telemetry_config = addWriteFileCommand(
        b,
        b.fmt("{s}/build_telemetry.cfg", .{depot_tools_dir}),
...
fn bootstrapV8(
    ...
    const write_gclient = addWriteFileCommand(
        b,
        b.fmt("{s}/.gclient", .{v8_dir}),
        gclient_content,
    );
    ...
    const write_gclient_args = addWriteFileCommand(
        b,
        b.fmt("{s}/build/config/gclient_args.gni", .{v8_dir}),
